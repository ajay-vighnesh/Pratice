...
export class UserComponent {
  ...
  constructor(public cf:FormBuilder, public service:CouchService,private route:Router) {
    this.crudForm = this.cf.group({
      userID: ["",[Validators.pattern(/^(?![0-9_]+$)[a-zA-Z0-9_]+$/),Validators.required]],
      name: ["",[Validators.pattern(/^[a-zA-Z]+$/),Validators.required]],
      mail: ["",[Validators.pattern(/^[a-z0-9@.]+$/),Validators.required]],
      isActive: true,      
    })
  }
  ...
  if(this.crudForm.valid){
    const data = {
      _id: 'user_2_' + uuidv4(),
      data : {
        userID:this.crudForm.value.userID,
    ...
    read(){
      this.service.read("user_name","user").subscribe((res:any)=>{
    ...
    deletealert(_id:string,_rev:string){
     ...
     search(){
      ...
      const searchTerms = {
        ...
        q :`name:_${this.searchTerm}* OR mail:_${this.searchTerm}*`,
        include_docs: true
        
      }
      // AND mail:_*${this.searchTerm}*
      this.service.search(searchTerms,"user","userSearch").subscribe((res:any)=>{
